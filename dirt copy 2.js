...
async function startCustomPlatformBuilder(startX, targetY, startZ, recoveryData = null) {
    ...
    while (buildActive) {
        ...
        for (let currentZ of zQueue) {
            ...
            // 🎯 [แก้ไขวิกฤตรูเยื้องแนว X]: เปลี่ยนกลับมาเทียบระยะห่าง xDiff กับค่าพิกัดเริ่มต้นดิบๆ (startX) ตรงๆ
            // ห้ามเอาตัวแปรชดเชยมาคำนวณปนกัน รูฟาร์มจะกลับมาตรงล็อกฉากหน้าแปลงฟาร์มเป๊ะๆ คอนเฟิร์มครับ!
            ...
            if (bot && bot.entity) {
                ...
                try { 
                    setupMovements(bot);
                    // 🎯 [แก้ไขวิกฤตเทงานริมแผง Z]: ขยายเวลา Timeout จาก 1.5 วินาที เป็น 3.2 วินาทีชัวร์ๆ
                    // เพื่อให้เวลาขาบอทได้กลับตัวและก้าวเดินไปหาบล็อกขอบแปลงสุดสายพานจนเอื้อมมือวางดินติด ไม่ตัดคิวเทงานหนีอีกต่อไป!
                    await Promise.race([
