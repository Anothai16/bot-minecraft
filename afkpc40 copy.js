...
function createBotInstance(username, delayMs = 0) {
    ...
    setTimeout(() => {
        ...
        bot.on('spawn', () => {
            ...
            if (bot.authStage === 'SURVIVAL_DONE') {
                log(`[⚠️ WARN] [${username}] ตรวจพบการถูกย้ายกลับ Lobby! กำลังเริ่มกระบวนการกดเข็มทิศใหม่...`);
                ...
                updateStatus(username, 'In Lobby', 'โดนเด้งกลับ Lobby (กำลังเลือกใหม่)');
        ...
        bot.on('message', (jsonMsg) => {
            ...
            if (rawText.includes('ย้ายคุณไปยัง') || rawText.includes('Lobby') || rawText.includes('เชื่อมต่อล้มเหลว')) {
                if (bot.authStage === 'SURVIVAL_DONE') {
                    log(`[📢 Server] [${username}]: ${rawText}`);
                    updateStatus(username, 'In Lobby', 'เซิร์ฟเวอร์ส่งกลับ Lobby');
                    if (bot.afkInterval) {
                        clearInterval(bot.afkInterval);
                        bot.afkInterval = null;
                    }
