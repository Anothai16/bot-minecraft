...
function getProxyPortForBot(botName) {
    ...
    if (index >= 0 && index < 10) return null; // 01-10: เน็ตบ้านตรงๆ
    if (index >= 10 && index < 20) return 1080; // 11-20: WARP vpn1
    if (index >= 20 && index < 30) return 1081; // 21-30: WARP vpn2
    if (index >= 30 && index < 40) return 1082; // 31-40: WARP vpn3
    if (index >= 40 && index < 50) return 1083; // 41-50: Tor Instance 1
    if (index >= 50 && index < 60) return 1084; // 51-60: Tor Instance 2
    if (index >= 60 && index < 70) return 1085; // 61-70: Tor Instance 3
    if (index >= 70 && index < 80) return 1086; // 71-80: Tor Instance 4
    return null;
}
...
// ฟังก์ชันคำนวณหน่วยย่อตัวเลข (เช่น 1.6K -> 1600, 2.5M -> 2500000)
...
function parseBitsFromString(rawString) {
    ...
    if (/บิท|bit/i.test(text)) {
        ...
        // เคสข้อความธรรมดา เช่น "บิท: 1.6K" หรือ "Bits 1.6k"
    ...
    setTimeout(() => {
        ...
        bot.on('end', (reason) => {
            ...
            if (botStatusMap[username]?.enabled) {
                ...
                log(`[i] [${username}] จะต่อใหม่ใน 45 วินาที...`);
                createBotInstance(username, 45000);
            } else {
        ...
        if (action === 'start-range') {
            ...
            targetBots.forEach((bName) => {
                ...
                if (!isRunning) {
                    ...
                    createBotInstance(bName, launchIndex * 14000); // ดีเลย์ 14 วิ เพื่อป้องกัน Anti-Spam
                    launchIndex++;
                }
            });
        }
            ...
            BOT_NAMES.forEach((bName) => {
                ...
                if (!isRunning) {
                    botStatusMap[bName].enabled = true;
                    createBotInstance(bName, launchIndex * 14000);
                    launchIndex++;
                }
            });
        } 
        ...
        async function fetchStatus() {
            try {
                ...
                document.getElementById('summary').innerHTML = 
                    \`ออนไลน์ทั้งหมด: <b>\${onlineCount}/\${total}</b> ตัว | บิทรวมทั้งหมด: <b style="color:#12dbf6">💎 \${totalBits.toLocaleString()} บิท</b> | (โหมด 512MB RAM + Multi-Proxy Ready)\`;
            } catch (e) {}
        }
